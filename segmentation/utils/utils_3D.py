...
def zero_pad_model_input_bacteria(img):
    """ Zero-pad model input to get for the model needed sizes.
    ...
    if img.shape[0] < 32:
        # Zero-pad to 32
        z_pads = 32 - img.shape[0]
    elif img.shape[0] == 32:
        z_pads = 0
    elif 32 < img.shape[0] < 64:
        z_pads = 64 - img.shape[0]
    elif img.shape[0] == 64:
        z_pads = 0 
    elif 64 < img.shape[0] < 128:
        z_pads = 128 - img.shape[0]
    elif img.shape[0] == 128:
        z_pads = 0 
    elif 128 < img.shape[0] < 256:
        z_pads = 256 - img.shape[0]
    elif img.shape[0] == 256:
        z_pads = 0 
    elif 256 < img.shape[0] < 512:
        z_pads = 512 - img.shape[0]
    elif img.shape[0] == 512:
        z_pads = 0 
    #   
    elif 512 < img.shape[0] < 768:
        z_pads = 768 - img.shape[0]
    elif img.shape[0] == 768:
        z_pads = 0
    elif 768 < img.shape[0] < 1024:
        z_pads = 1024 - img.shape[0]
    elif img.shape[0] == 1024:
        z_pads = 0
    elif 1024 < img.shape[0] < 1280:
        z_pads = 1280 - img.shape[0]
    elif img.shape[0] == 1280:
        z_pads = 0        
    else:
        raise Exception('Padding error. Image too big?')  
    ...
    if img.shape[1] < 32:
        # Zero-pad to 32
        y_pads = 32 - img.shape[1]
    elif img.shape[1] == 32:
        y_pads = 0
    elif 32 < img.shape[1] < 64:
        y_pads = 64 - img.shape[1]
    elif img.shape[1] == 64:
        y_pads = 0 
    elif 64 < img.shape[1] < 128:
        y_pads = 128 - img.shape[1]
    elif img.shape[1] == 128:
        y_pads = 0 
    elif 128 < img.shape[1] < 256:
        y_pads = 256 - img.shape[1]
    elif img.shape[1] == 256:
        y_pads = 0 
    elif 256 < img.shape[1] < 512:
        y_pads = 512 - img.shape[1]
    elif img.shape[1] == 512:
        y_pads = 0 
      #  
    elif 512 < img.shape[1] < 768:
        y_pads = 768 - img.shape[1]
    elif img.shape[1] == 768:
        y_pads = 0
    elif 768 < img.shape[1] < 1024:
        y_pads = 1024 - img.shape[1]
    elif img.shape[1] == 1024:
        y_pads = 0
    elif 1024 < img.shape[1] < 1280:
        y_pads = 1280 - img.shape[1]
    elif img.shape[1] == 1280:
        y_pads = 0       
    else:
        raise Exception('Padding error. Image too big?')  
    ...
    if img.shape[2] < 32:
        # Zero-pad to 32
        x_pads = 32 - img.shape[2]
    elif img.shape[2] == 32:
        x_pads = 0
    elif 32 < img.shape[2] < 64:
        x_pads = 64 - img.shape[2]
    elif img.shape[2] == 64:
        x_pads = 0 
    elif 64 < img.shape[2] < 128:
        x_pads = 128 - img.shape[2]
    elif img.shape[2] == 128:
        x_pads = 0 
    elif 128 < img.shape[2] < 256:
        x_pads = 256 - img.shape[2]
    elif img.shape[2] == 256:
        x_pads = 0 
    elif 256 < img.shape[2] < 512:
        x_pads = 512 - img.shape[2]
    elif img.shape[2] == 512:
        x_pads = 0 
       # 
    elif 512 < img.shape[2] < 768:
        x_pads = 768 - img.shape[2]
    elif img.shape[2] == 768:
        x_pads = 0
    elif 768 < img.shape[2] < 1024:
        x_pads = 1024 - img.shape[2]
    elif img.shape[2] == 1024:
        x_pads = 0
    elif 1024 < img.shape[2] < 1280:
        x_pads = 1280 - img.shape[2]
    elif img.shape[2] == 1280:
        x_pads = 0           
    else:
        raise Exception('Padding error. Image too big?')  
    
    img = np.pad(img, ((z_pads, 0), (y_pads, 0), (x_pads, 0), (0, 0)), mode='constant')

    return img, [z_pads, y_pads, x_pads]

def find_block_size(img):
    if img.shape[0]==32:
        block_size_z=32
    elif img.shape[0]>=64:
        block_size_z=64
    ...
    return block_size_z, block_size_y, block_size_x
