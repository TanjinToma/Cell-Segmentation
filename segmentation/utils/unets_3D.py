...
def build_unet(unet_type,
  ...
  # U-Net with two decoder paths and two single channel outputs (e.g., cell + neighbor dist)
    ...
    # if print_path is not None:  # Plot an save graph of the model
    #     hl_graph = hl.build_graph(model, torch.zeros([1, ch_in, 256, 256]))
    #     dot = hl_graph.build_dot()
    #     dot.attr("graph", rankdir="TD")  # Top-Down
    #     # dot.attr("graph", rankdir="LR")  # Left-Right
    #     hl_graph.save(print_path / (unet_type + '_' + act_fun + '_' + normalization + '_' + pool_method + '.pdf'))
...
class DUNet(nn.Module):
    ...
    def __init__(self, ch_in=1, ch_out=1, pool_method='conv', act_fun='relu', normalization='bn', filters=(64, 1024)):
        ...
        self.decoder1Conv.append(nn.Conv3d(n_featuremaps, ch_out, kernel_size=1, stride=1, padding=0))
