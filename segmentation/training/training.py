...
from segmentation.training.losses_3D import get_loss
from segmentation.training.losses_3D import msssim_loss
            ...
            for samples in dataloader[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if configs['label_type'] in ['boundary', 'border', 'pena']:
                        ...
                        border_pred_batch, cell_pred_batch = net(img_batch)
                        loss_border_sl1 = criterion['border'](border_pred_batch, border_label_batch)
                        loss_cell_sl1 = criterion['cell'](cell_pred_batch, cell_label_batch)
                        loss_border_sm = msssim_loss(border_pred_batch, border_label_batch)
                        loss_cell_sm = msssim_loss(cell_pred_batch, cell_label_batch)
                        loss_border = (0.5 * loss_border_sl1) + (0.5 * loss_border_sm)
                        loss_cell = (0.5 * loss_cell_sl1) + (0.5 * loss_cell_sm)
                        #loss = loss_border_sl1 + loss_cell_sl1
